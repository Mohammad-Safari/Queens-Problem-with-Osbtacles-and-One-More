...
def check_danger(queen: tuple, row: int, col: int) -> bool:
    def is_path_blocked_by_obstacle(
        ...
        if House.Obstacle in [
            board[row + step[0] * i][col + step[1] * i]
            for i in range(max(abs(rd), abs(cd)))
        ]:
            return True
        return False
...
def dfs(placed_queens: list):
    if len(placed_queens) == queen_numbers and not detect_existence(placed_queens):
        ...
        print(str(calculate_board(board,scores))+'-', end="")
        return True
    ...
    for safe_house in safe_houses:
        ...
        # ):
        if ([placed_queens[-1]]+get_safe_houses((placed_queens[:-1] if len(placed_queens)>1 else []))).index(placed_queens[-1]) if len(placed_queens) > 0 else -1 > safe_houses.index(safe_house):
            continue
